...
from schemas import AuctionCreate, AuctionUpdate, UserCreate, UserLogin, UserResponse
...
def get_user(db: Session, username: str, email: str) -> Optional[UserModel]:
...
def create_user(db: Session, username: str, email: str, password: str) -> UserModel:
    exist_user = get_user(db, username, email)
    if exist_user:
